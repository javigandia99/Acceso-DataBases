...
if(isset($parameters)){
	...
	if(JSONCorrectoAnnadir($recibido)){
		$user = $recibido["userDel"]; 
        
        $username = $user["username"];
        $query = "DELETE FROM users WHERE users.db_username = '$username'";
		...
		if (isset ( $result ) && $result) { // Si pasa por este if, la query está está bien y se ha borrado correctamente
			...
			$arrMessage["state"] = "ok";
			$arrMessage["message"] = "$username borrado correctamente";
		...
		$arrMessage["esperado"] = $arrEsperadoDel;
	}
