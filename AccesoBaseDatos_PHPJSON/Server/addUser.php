...
if(isset($parameters)){
	...
	if(JSONCorrectoAnnadir($recibido)){
		...
		$user = $recibido["userAdd"]; 
		
		$username = $user["username"];
		$password = $user["password"];
		$description = $user["description"];
		
		$query  = "INSERT INTO  users (db_username,db_password,db_description) ";
		$query .= "VALUES ('$username','$password','$description')";
		...
		if (isset ( $result ) && $result) { // Si pasa por este if, la query está está bien y se ha insertado correctamente
			...
			$arrMessage["state"] = "ok";
			$arrMessage["message"] = "Jugador insertado correctamente";
		...
		$arrMessage["esperado"] = $arrEsperadoAdd;
	}
